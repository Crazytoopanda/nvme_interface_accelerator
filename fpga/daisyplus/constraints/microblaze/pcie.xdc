...
# NVMe/PCIe CDC constraints.
# pcie_user_rst_n_raw is generated outside user_clk_out and is synchronised in user_top.
set_property ASYNC_REG TRUE [get_cells -hier -quiet {*r_pcie_user_rst_n_sync_reg[*]}]
set_false_path -to [get_pins -hier -quiet {*r_pcie_user_rst_n_sync_reg[*]/CLR}]

# The clk_wiz control/DDR clocks and PCIe user clock are asynchronous.
# CDC logic in nvme_ctrl handles these crossings; do not time them as synchronous paths.
set_clock_groups -asynchronous \
    -group [get_clocks -quiet {clk_out1_qkv_accelerator_clk_wiz_0_0}] \
    -group [get_clocks -quiet {clk_out2_qkv_accelerator_clk_wiz_0_0}] \
    -group [get_clocks -quiet {user_clk_out GTHE4_CHANNEL_TXOUTCLK*}]
