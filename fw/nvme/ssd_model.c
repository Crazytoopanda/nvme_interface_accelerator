...
#define SSD_MODEL_BUFFER_RELEASE_SLOTS		SSD_MODEL_CMD_SLOT_COUNT
...
#define SSD_MODEL_TOTAL_LUNS			SSD_MODEL_NUM_LANES
...
#define SSD_MODEL_PAGES_PER_LINE		(SSD_MODEL_PAGES_PER_BLOCK * SSD_MODEL_TOTAL_LUNS)
#define SSD_MODEL_LINE_COUNT			BLKS_PER_PLN
#define SSD_MODEL_LOGICAL_PAGES			(NVME_STORAGE / BYTES_PER_NVME_BLOCK)
#define SSD_MODEL_PHYSICAL_PAGES		((SSD_MODEL_LINE_COUNT * 1ULL) * \
...
#define SSD_MODEL_META_END_ADDR			(SSD_MODEL_P2L_ADDR + \
...
static unsigned short g_lineValidPages[SSD_MODEL_LINE_COUNT];
static unsigned short g_lineInvalidPages[SSD_MODEL_LINE_COUNT];
static unsigned char g_lineState[SSD_MODEL_LINE_COUNT];
static unsigned int g_freeLineCount;
static SSD_MODEL_WRITE_POINTER g_userWritePointer;
static unsigned int g_writeCredits;
static unsigned int g_creditsToRefill;
...
{
	unsigned int idx;
	unsigned long long earliestNs = ~0ULL;

	for(idx = 0; idx < SSD_MODEL_BUFFER_RELEASE_SLOTS; idx++)
	{
		if(g_writeBufferReleaseBytes[idx] != 0 &&
		   g_writeBufferReleaseNs[idx] < earliestNs)
			earliestNs = g_writeBufferReleaseNs[idx];
	}
						 ...
						 unsigned long long bytes)
{
	...
	for(idx = 0; idx < SSD_MODEL_BUFFER_RELEASE_SLOTS; idx++)
	{
		if(g_writeBufferReleaseBytes[idx] == 0)
		{
			g_writeBufferReleaseBytes[idx] = bytes;
			g_writeBufferReleaseNs[idx] = doneNs;
			return;
		}
	}
...
{
	return ppa % SSD_MODEL_TOTAL_LUNS;
}
...
static unsigned int ssd_model_same_flash_page(unsigned int a, unsigned int b)
{
	...
	return (a / (SSD_MODEL_TOTAL_LUNS * SSD_MODEL_PAGES_PER_ONESHOT)) ==
	       (b / (SSD_MODEL_TOTAL_LUNS * SSD_MODEL_PAGES_PER_ONESHOT));
}

static unsigned long long ssd_model_read_latency_ppa_ns(unsigned int ppa,
						 unsigned long long xferBytes)
{
	if(((ppa / SSD_MODEL_TOTAL_LUNS) & 1U) != 0)
						   ...
						   unsigned long long xferBytes)
{
	...
	chDoneNs = chStartNs + FW_CH_XFER_LATENCY +
		   ssd_model_xfer_ns(xferBytes, NAND_CHANNEL_BANDWIDTH);
						    ...
						    unsigned long long bytes)
{
	...
	chStartNs = ssd_model_max_ull(startNs, g_chAvailNs[ch]);
	chDoneNs = chStartNs + FW_CH_XFER_LATENCY +
		   ssd_model_xfer_ns(bytes, NAND_CHANNEL_BANDWIDTH);
...
static unsigned int ssd_model_open_free_line(SSD_MODEL_WRITE_POINTER *wp)
{
	unsigned int line;

	for(line = 0; line < SSD_MODEL_LINE_COUNT; line++)
	{
		if(g_lineState[line] == SSD_MODEL_LINE_FREE)
		{
			g_lineState[line] = SSD_MODEL_LINE_OPEN;
			if(g_freeLineCount != 0)
				g_freeLineCount--;
			wp->line = line;
			wp->page = 0;
			wp->lun = 0;
...
static void ssd_model_advance_write_pointer(SSD_MODEL_WRITE_POINTER *wp)
{
	unsigned int line = wp->line;
	...
	g_lineState[line] = g_lineInvalidPages[line] ?
			     SSD_MODEL_LINE_VICTIM : SSD_MODEL_LINE_FULL;
	ssd_model_write_pointer_clear(wp);
}

static unsigned int ssd_model_write_pointer_ppa(const SSD_MODEL_WRITE_POINTER *wp)
{
	return (wp->line * SSD_MODEL_PAGES_PER_LINE) +
	       (wp->page * SSD_MODEL_TOTAL_LUNS) +
	       (wp->lun * NAND_CHANNELS) + wp->ch;
}

static unsigned int ssd_model_program_ready_ppa(unsigned int ppa)
{
	unsigned int page = (ppa % SSD_MODEL_PAGES_PER_LINE) / SSD_MODEL_TOTAL_LUNS;

	return (page % SSD_MODEL_PAGES_PER_ONESHOT) ==
...
static void ssd_model_invalidate_lpn(unsigned int lpn)
{
	...
	line = oldPpa / SSD_MODEL_PAGES_PER_LINE;
	if(g_lineValidPages[line] != 0)
		g_lineValidPages[line]--;
	g_lineInvalidPages[line]++;
	if(g_lineState[line] == SSD_MODEL_LINE_FULL)
		g_lineState[line] = SSD_MODEL_LINE_VICTIM;

	p2l[oldPpa] = SSD_MODEL_INVALID_PAGE;
	l2p[lpn] = SSD_MODEL_INVALID_PAGE;
}

static unsigned int ssd_model_allocate_ppa(unsigned int lpn)
{
	unsigned int *l2p = ssd_model_l2p();
	unsigned int *p2l = ssd_model_p2l();
	unsigned int ppa;
	...
	if(g_userWritePointer.line == SSD_MODEL_INVALID_PAGE &&
	   ssd_model_open_free_line(&g_userWritePointer) == 0)
		return SSD_MODEL_INVALID_PAGE;

	ppa = ssd_model_write_pointer_ppa(&g_userWritePointer);
	if(ppa >= SSD_MODEL_PHYSICAL_PAGES)
		return SSD_MODEL_INVALID_PAGE;

	p2l[ppa] = lpn;
	l2p[lpn] = ppa;
	g_lineValidPages[g_userWritePointer.line]++;
	ssd_model_advance_write_pointer(&g_userWritePointer);
	return ppa;
}

static unsigned int ssd_model_select_gc_victim(void)
{
	unsigned int line;
	...
	for(line = 0; line < SSD_MODEL_LINE_COUNT; line++)
	{
		if(g_lineState[line] != SSD_MODEL_LINE_VICTIM)
			continue;
		if(g_lineValidPages[line] < bestValid)
		{
			bestValid = g_lineValidPages[line];
			victim = line;
		}
	}
...
static unsigned long long ssd_model_refill_write_credit(unsigned long long startNs);

static unsigned long long ssd_model_reclaim_one_line(unsigned long long startNs)
{
	unsigned int *l2p = ssd_model_l2p();
	unsigned int *p2l = ssd_model_p2l();
	unsigned int victim = ssd_model_select_gc_victim();
	unsigned int idx;
	...
	{
		unsigned int oldPpa = victim * SSD_MODEL_PAGES_PER_LINE + idx;
		...
		readDoneNs = ssd_model_schedule_nand_read_ppa(oldPpa, startNs, BYTES_PER_NVME_BLOCK);
		newPpa = ssd_model_allocate_ppa(lpn);
		...
		p2l[oldPpa] = SSD_MODEL_INVALID_PAGE;
		writeDoneNs = ssd_model_schedule_nand_write_ppa(newPpa, readDoneNs,
							 ONESHOT_PAGE_SIZE);
		latestNs = ssd_model_max_ull(latestNs, writeDoneNs);
	}

	g_lineValidPages[victim] = 0;
	g_lineInvalidPages[victim] = 0;
	g_lineState[victim] = SSD_MODEL_LINE_FREE;
	g_freeLineCount++;
	g_ssdModelDebug[15]++;
...
static unsigned long long ssd_model_refill_write_credit(unsigned long long startNs)
{
	...
	if(g_freeLineCount <= 2)
		latestNs = ssd_model_reclaim_one_line(startNs);

	g_writeCredits += g_creditsToRefill;
	return latestNs;
}
...
static void ssd_model_ftl_reset(void)
{
	...
	for(idx = 0; idx < SSD_MODEL_LINE_COUNT; idx++)
	{
	...
	g_freeLineCount = SSD_MODEL_LINE_COUNT;
	ssd_model_write_pointer_clear(&g_userWritePointer);
	g_writeCredits = SSD_MODEL_PAGES_PER_LINE;
	g_creditsToRefill = SSD_MODEL_PAGES_PER_LINE;
}
...
static void ssd_model_schedule_io(SSD_MODEL_IO *io)
{
	...
	if(io->op == SSD_MODEL_OP_READ)
	{
		...
		readStartNs = nowNs + ((io->requestedNvmeBlock <= 1) ?
					     FW_4KB_READ_LATENCY : FW_READ_LATENCY);
		latestNs = readStartNs;
		pageIdx = 0;
		while(pageIdx < io->requestedNvmeBlock)
		{
			...
			while(pageIdx < io->requestedNvmeBlock)
			{
				...
				unsigned int nextPpa;

				pageAddr = io->devAddr +
					   ((unsigned long long)pageIdx * BYTES_PER_NVME_BLOCK);
				nextLpn = ssd_model_lpn_from_addr(pageAddr);
				if(nextLpn >= SSD_MODEL_LOGICAL_PAGES)
					break;
	...
	{
		writeNandLatestNs = ssd_model_max_ull(writeNandLatestNs,
						 ssd_model_refill_write_credit(writeBufferDoneNs));
